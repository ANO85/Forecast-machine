...
def exponential_smoothing_forecast(group, months_to_forecast=12):
    try:
        ...
        if len(monthly_sales) < 2:
            avg_sales = monthly_sales['Qty'].mean()
            es_forecast = [avg_sales] * months_to_forecast
        else:
...
def linear_regression_forecast(group, months_to_forecast=12):
    try:
        # Groupement par mois
        monthly_sales = group.groupby(pd.Grouper(key='Date', freq='M'))['Qty'].sum().reset_index()
        ...
        if len(monthly_sales) < 2:
            avg_sales = monthly_sales['Qty'].mean()
            lr_forecast = [avg_sales] * months_to_forecast
        else:
            ...
            lr_forecast = [slope * (last_index + i) + intercept for i in range(months_to_forecast)]
        ...
        if df is not None:
            ...
            for group in customer_groups:
                for item in items:
                    ...
                    lr_forecast = linear_regression_forecast(subset)
