...
if uploaded_file is not None:
    try:
        ...
        if not required_cols.issubset(df.columns):
            ...
            # **Suppression explicite des doublons après agrégation et réinitialisation de l'index**
            df = df.drop_duplicates(subset=['Date', 'Item', 'Customer Group']).reset_index(drop=True)
...
def exponential_smoothing_forecast(series):
    try:
        ...
        model_fit = model.fit()
        return model_fit.forecast(12)  # Retourne la série de prévisions
    except Exception as e:
        return pd.Series([np.nan] * 12), str(e)  # Retourne aussi le message d'erreur en cas d'exception
    ...
    try:
        ...
        if not all(col in df.columns for col in expected_cols):
            ...
            for keys, group in grouped:
                ...
                # **Réinitialisation explicite de l'index pour chaque série**
                series = series[~series.index.duplicated(keep='first')]  # Supprime les doublons dans l'index
                series = series.reset_index(drop=False)  # Réinitialise l'index à nouveau après traitement

                # Prévisions des 12 mois à venir
                ma_6 = moving_average_forecast(series['Qty'], 6).dropna().iloc[-1] if len(series) >= 6 else np.nan
                ma_12 = moving_average_forecast(series['Qty'], 12).dropna().iloc[-1] if len(series) >= 12 else np.nan
                exp, exp_error = exponential_smoothing_forecast(series['Qty'])

                if isinstance(exp, pd.Series):  # Si exp est une série de prévisions
                    for i, date in enumerate(pd.date_range(series['Date'].iloc[-1] + pd.DateOffset(months=1), periods=12, freq='MS')):
                        forecast_exp.append([*keys, date, exp[i]])

                else:
                    st.error(f"Erreur dans la prévision exponentielle pour l'item {keys}: {exp_error}")

                future_dates = pd.date_range(series['Date'].iloc[-1] + pd.DateOffset(months=1), periods=12, freq='MS')
                ...
                if len(series) >= 15:
                    last_12 = series['Qty'].iloc[-15:-3]
                    true_vals = series['Qty'].iloc[-3:]
